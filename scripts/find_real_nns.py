...
def main(options):
    ...
    for tree in trees:
        ...
        for node_1 in terminal_nodes:
            label_1=int(float(node_1.name))-1
            for node_2 in terminal_nodes:
                label_2=int(float(node_2.name))-1
                if label_1>=label_2:
                    continue
        ...
        for node in internal_nodes:
            if node.is_preterminal(): 
                labels=[int(float(x.name))-1 for x in node.get_terminals()] 
                labels.sort()
...
def clade_terminals_changed(tree, last_tree, label_1, label_2, options):
    ...
    last_subclade_labels=set([int(float(x.name))-1 for x in last_subclade_nodes])
    this_subclade_nodes=tree.common_ancestor(node_1,node_2).get_terminals()
    this_subclade_labels=set([int(float(x.name))-1 for x in this_subclade_nodes])
...
def find_tmrca_and_subclade(tree, options):
    ...
    for node in tree.get_nonterminals():
        ...
        #labels=set([conf2label(x.name, options) for x in leaves] )
        labels=set([int(float(x.name))-1 for x in leaves] )
